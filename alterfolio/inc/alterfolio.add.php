...
list($usr['auth_read'], $usr['auth_write'], $usr['isadmin']) = cot_auth('alperfolio', 'any', 'RWA');
...
if ($a == 'add')
{
	...
	list($usr['auth_read'], $usr['auth_write'], $usr['isadmin']) = cot_auth('alperfolio', $ritem['item_cat']);
	cot_block($usr['auth_write']);
...
{
	$ritem['item_cat'] = $c;
	$usr['isadmin'] = cot_auth('alperfolio', $ritem['item_cat'], 'A');
}
